...
<RevealCard className="mt-8">
  <Card className="flex flex-col items-center justify-center py-8 relative">
    ...
    <div className="mb-6 relative w-24 h-24 flex items-center justify-center">
      ...
      {/* --- TONEARM (Now correctly positioned on the grooves) --- */}
      <div className="absolute top-0 right-1 z-20 w-full h-full pointer-events-none">
        
        {/* Pivot Base (Top Right Corner) */}
        <div className="absolute top-0 right-0 w-6 h-6 rounded-full bg-zinc-800 border-2 border-zinc-700 shadow-xl flex items-center justify-center z-20">
            <div className="w-2 h-2 bg-zinc-400 rounded-full shadow-inner"></div>
        </div>
        
        {/* The Arm (Swings inward to hit the record) */}
        <div 
          className="absolute top-3 right-3 w-1.5 h-16 bg-zinc-700 origin-top rounded-full border-r border-zinc-600/50 shadow-lg"
          style={{ transform: 'rotate(-35deg)' }} // Fixed rotation to land on vinyl
        >
          {/* The Headshell (Cartridge) */}
          <div className="absolute bottom-0 -left-1.5 w-4 h-6 bg-zinc-800 rounded-sm border border-zinc-600 flex justify-center items-end">
             {/* The Needle Tip (Touching the record) */}
      ...
      <div className="w-20 h-20 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center animate-[spin_3s_linear_infinite] shadow-lg relative z-10">
        {/* Vinyl Grooves */}
        <div className="absolute inset-2 rounded-full border border-zinc-800 opacity-50"></div>
        <div className="absolute inset-4 rounded-full border border-zinc-800 opacity-50"></div>
        <div className="absolute inset-6 rounded-full border border-zinc-800 opacity-50"></div>
